...
let availableCommands = [
  ...
  },
  {
    ...
    usage: "!set",
    description:
      "Open a multi-line editor to create or edit the system prompt. When changes are saved and the editor is closed, the system message will be updated.",
    ...
    command: async (args) => {
      ...
      if (args.length === 1) {
        ...
        if (clientOptions.messageOptions.systemMessage) {
          console.log(
            systemMessageBox(clientOptions.messageOptions.systemMessage)
          );
        }
      ...
      try {
        ...
        if (
          clientOptions.messageOptions.content &&
          clientOptions.messageOptions.content.startsWith(
            "Hello, how can I assist you today?"
          )
        ) {
          clientOptions.messageOptions.content =
            clientOptions.messageOptions.content
              .replace("Hello, how can I assist you today?", "")
              .trim();
        }
  ...
  },
  {
    ...
    command: async (args) => {
      ...
      if (!MODEL_INFO[newModel]) {
        logWarning(
          `Model "${newModel}" not found. Available models: ${Object.keys(
            MODEL_INFO
          ).join(", ")}`
        );
        return conversation();
      ...
      // Save the current system message before switching
      ...
      // Create safe copies of the default options with special handling for Gemini
      const createSafeModelOptions = (defaultOptions) => {
        // Make a deep copy
        const options = JSON.parse(JSON.stringify(defaultOptions));

        // If this is Gemini, special handling needed
        if (modelConfig.company === "google") {
          // Remove any 'stream' parameter from modelOptions that Gemini doesn't support
          if (options.modelOptions && "stream" in options.modelOptions) {
            delete options.modelOptions.stream;
          }
        }
      ...
      // Update model-specific options but preserve system message
      if (modelConfig.company === "anthropic") {
        // Create a deep copy of default options with safe transformations
        settings.cliOptions.claudeOptions = createSafeModelOptions(
          modelConfig.defaultOptions
        );

        // Preserve system message if one exists
        if (currentSystemMessage) {
          settings.cliOptions.claudeOptions.messageOptions =
            settings.cliOptions.claudeOptions.messageOptions || {};
          settings.cliOptions.claudeOptions.messageOptions.systemMessage =
            currentSystemMessage;
        }
      } else if (modelConfig.company === "openai") {
        // Create a deep copy of default options with safe transformations
        settings.cliOptions.chatGptOptions = createSafeModelOptions(
          modelConfig.defaultOptions
        );

        // Preserve system message if one exists
        if (currentSystemMessage) {
          settings.cliOptions.chatGptOptions.messageOptions =
            settings.cliOptions.chatGptOptions.messageOptions || {};
          settings.cliOptions.chatGptOptions.messageOptions.systemMessage =
            currentSystemMessage;
        }
      } else if (modelConfig.company === "microsoft") {
        // Create a deep copy of default options with safe transformations
        settings.cliOptions.bingOptions = createSafeModelOptions(
          modelConfig.defaultOptions
        );

        // Preserve system message if one exists
        if (currentSystemMessage) {
          settings.cliOptions.bingOptions.messageOptions =
            settings.cliOptions.bingOptions.messageOptions || {};
          settings.cliOptions.bingOptions.messageOptions.systemMessage =
            currentSystemMessage;
        }
      } else if (modelConfig.company === "google") {
        // Create a deep copy of default options with safe transformations
        settings.cliOptions.geminiOptions = createSafeModelOptions(
          modelConfig.defaultOptions
        );

        // Preserve system message if one exists
        if (currentSystemMessage) {
          settings.cliOptions.geminiOptions.messageOptions =
            settings.cliOptions.geminiOptions.messageOptions || {};
          settings.cliOptions.geminiOptions.messageOptions.systemMessage =
            currentSystemMessage;
        }
      }
      ...
      // Make sure the new client has the system message set properly
      if (currentSystemMessage) {
        // Update system message in client options
        clientOptions.messageOptions = clientOptions.messageOptions || {};
        clientOptions.messageOptions.systemMessage = currentSystemMessage;
        clientOptions.system = currentSystemMessage;

        // Update client's internal options
        client.options = client.options || {};
        client.options.system = currentSystemMessage;

        if (client.options.messageOptions) {
          client.options.messageOptions.systemMessage = currentSystemMessage;
        }
      }
      ...
      // Display the system message if one is set
      if (currentSystemMessage) {
        console.log(
          systemMessageBox(currentSystemMessage, "System (Preserved)")
        );
      }
    ...
    command: async (args) => {
      ...
      try {
        ...
        if (!seedData.messages || !Array.isArray(seedData.messages)) {
          throw new Error(
            "Invalid seed file structure: missing messages array"
          );
        }
        ...
        for (const msg of seedData.messages) {
          ...
          const conversationMessage = {
            ...
            role:
              msg.role ||
              (msg.author === client.names.bot.author
                ? client.names.bot.role
                : client.names.user.role),
        ...
        await setConversationData({
          conversationId,
          parentMessageId, // Point to last message
        });
...
async function generateMessage() {
  ...
  const context = await client.yieldGenContext(
    ...
    },
    {
      ...
      // Make sure the system message is passed directly to prioritize it
      systemMessage:
        clientOptions.messageOptions?.systemMessage || clientOptions.system,
    }
  );
...
async function useSystemEditor() {
  ...
  logSuccess(`System message updated for ${getAILabel()}`);
...
function convertRole(role, company) {
  if (!role) {
    // Handle null or undefined roles by defaulting to 'user'
    console.log(
      chalk.yellow('Warning: Encountered undefined role, defaulting to "user"')
    );
    return "user";
  }

  // Convert to standard roles first
  let standardRole = String(role).toLowerCase();

  // Map all assistant-like roles to 'assistant'
  if (
    standardRole === "claude" ||
    standardRole === "assistant" ||
    standardRole === "bing" ||
    standardRole === "gemini" ||
    standardRole === "chatgpt" ||
    standardRole === "gpt" ||
    standardRole === "model" ||
    standardRole === "bot" ||
    // Add any other assistant-like roles
    /^assistant-/.test(standardRole)
  ) {
    standardRole = "assistant";
  }

  // Map all user-like roles to 'user'
  if (
    standardRole === "user" ||
    standardRole === "human" ||
    standardRole === "customer" ||
    // Add any other user-like roles
    /^user-/.test(standardRole)
  ) {
    standardRole = "user";
  }

  // Map all system-like roles to 'system'
  if (
    standardRole === "system" ||
    standardRole === "context" ||
    standardRole === "instruction"
  ) {
    standardRole = "system";
  }
  ...
  switch (company) {
    case "anthropic":
      ...
      // Default to user if the role doesn't match known roles
      return "user";
    case "openai":
      if (standardRole === "assistant") return "assistant";
      if (standardRole === "user") return "user";
    ...
    case "google":
      ...
      return "user";
    default:
      // If company is unknown, return the standardized role
      return standardRole === "assistant" ||
        standardRole === "user" ||
        standardRole === "system"
        ? standardRole
        : "user"; // Default to user for any unrecognized role
  }
}

async function generateBackroomsResponse(
  backroomsClient,
  options,
  userMessage,
  systemPrompt,
  ...
  try {
    ...
    const apiParams = {
      ...backroomsClient.buildApiParams(
        backroomsClient.buildMessage(
          userMessage.content,
          backroomsClient.names.user.author
        ),
        [],
        systemPrompt
          ? backroomsClient.buildMessage(systemPrompt, "system")
          : null
      ),
    ...
    const response = await backroomsClient.callAPI(apiParams, {
    ...
    if (response?.replies && response.replies[0]) {
...
async function startBackroomsDialogue(modelKey, contextName) {
  ...
  if (!MODEL_INFO[modelKey]) {
    logError(
      `Invalid model: ${modelKey}. Available models: ${Object.keys(
        MODEL_INFO
      ).join(", ")}`
    );
    return conversation();
