...
export function registerDreamSim(server, settings) {
  const clientToUse =
    settings.apiOptions?.clientToUse || settings.clientToUse || "openrouter";
  ...
  const manager = createSessionManager({
    ...
    settings,
    systemMessage:
      settings?.cliOptions?.openRouterOptions?.messageOptions?.systemMessage,
  });
  ...
  server.get("/css/:file", async (request, reply) => {
    try {
      const file = await fsp.readFile(
        path.resolve("web/css", request.params.file),
        "utf8"
      );
  ...
  server.get("/v1/dreamsim/history", async (request, reply) => {
    ...
    const sessionId = qid || "local";
    try {
      const { conversationId, cursorId, messages } = await manager.getHistory(
        sessionId
      );
