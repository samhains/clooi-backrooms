 ...
 */
export function conversationMessageBox(conversationMessage, ctx, index = null) {
    ...
    let float = 'center';
    if (hasComplexFormatting || aiMessage) {
        float = 'left';
    } else if (userMessage) {
        float = 'right';
    }
...
export function historyBoxes(messageHistory, ctx) {
    return messageHistory?.map((m, index) => conversationMessageBox(m, ctx, index)).join('\n');
}
